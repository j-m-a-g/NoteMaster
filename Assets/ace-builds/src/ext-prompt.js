  ...
  var AcePopup = /** @class */ (function () {
    function AcePopup(parentNode) {
      ...
      popup.renderer.on("afterRender", function () {
        ...
        for (
          var row = t.config.firstRow, l = t.config.lastRow;
          row <= l;
          row++
        ) {
          var popupRowElement = /** @type {HTMLElement|null} */ (
            t.element.childNodes[row - t.config.firstRow]
          );
          popupRowElement.setAttribute("role", optionAriaRole);
          popupRowElement.setAttribute(
            "aria-roledescription",
          ...
          if (rowData) {
            var ariaLabel = ""
              .concat(rowData.caption || rowData.value)
              .concat(rowData.meta ? ", ".concat(rowData.meta) : "");
            popupRowElement.setAttribute("aria-label", ariaLabel);
          }
          var highlightedSpans = popupRowElement.querySelectorAll(
...
define("ace/autocomplete/inline_screenreader", [
  "require",
  "exports",
  "module"
], function (require, exports, module) {
...
define("ace/autocomplete/inline", [
  "require",
  "exports",
  "module",
  "ace/snippets",
...
define("ace/autocomplete", [
  ...
  var Autocomplete = /** @class */ (function () {
    ...
    Autocomplete.prototype.showDocTooltip = function (item) {
      ...
      var topSize = popup.isTopdown
        ? rect.top
        : window.innerHeight - scrollBarSize - rect.bottom;
      var scores = [
      ...
      if (max == scores[0]) {
        ...
        tooltipStyle.maxHeight =
          Math.min(
            window.innerHeight - scrollBarSize - rect.top,
            targetHeight
          ) + "px";
        ...
        if (popup.isTopdown) {
          ...
          tooltipStyle.bottom = "";
          tooltipStyle.maxHeight =
            Math.min(popup.container.offsetTop, targetHeight) + "px";
        }
      }
    };
    ...
    prompt(editor, "", {
      ...
      getCompletions: function (cmdLine) {
        ...
        if (recentlyUsedCommands.length && otherCommands.length) {
          recentlyUsedCommands[0].message = nls(
            "prompt.recently-used",
            "Recently used"
          );
          otherCommands[0].message = nls(
